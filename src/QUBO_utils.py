...
def qubo_dict_to_torch(nx_G, Q, torch_dtype=None, torch_device=None):
    ...
    max_nodes=max(nx_G.nodes)+1
    # get QUBO Q as torch tensor
    #Q_mat = torch.zeros(n_nodes, n_nodes)
    Q_mat = torch.zeros(max_nodes, max_nodes)
...
def run_gnn_training(q_torch, dgl_graph, net, embed, optimizer, number_epochs, tol, patience, prob_threshold, file, params):
    """
    ...
    name = params["plot_path"]+'Hist_' + file[:-4] + '.png'
    plt.hist(probs.detach(), bins=np.linspace(0, 1, 50))
    plt.savefig(name)
    plt.show()
