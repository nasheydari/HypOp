...
path='../log/Hypermaxcut_syn_new.log'
with open(path) as f:
    Log=f.readlines()
...
for lines in Log:
    ...
    path="./data/hypergraph_data/synthetic/new/"+name
    with open(path) as f:
        file = f.read()
    lines = file.split('\n')
    info = lines[0].split(' ')
...
path='../log/Hypermaxcut_syn_new_Rand_l2.log'
...
plt.savefig('../res/plots/Hypermaxcut_syn.png')
plt.show()
...
plt.scatter(plotms[0,:], plotms[10,:], color='b', label='ADAM')
plt.scatter(plotms[0,:], plotms[3,:], color='g', label='HypOp')
plt.plot(x_model, y_model2, color='y', label='Linear Curve Fit for HypOp')
